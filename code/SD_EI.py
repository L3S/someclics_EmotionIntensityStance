...
for train_index,test_index in kf.split(total_sequence,label_enc):
    ...
    #### shared output ##############
    shared_input=Average()([IA_stance,IA_int])
    ...
    model=Model([input_shared,input_affect],[stance_output,int_output])


    ##Compile
    model.compile(optimizer=Adam(0.001),loss={'task_stance':'categorical_crossentropy','task_int':'mse'},
    loss_weights={'task_stance':1.0,'task_int':0.3},metrics=['accuracy'])    
    print(model.summary())
    ...
    model.fit([train_sequence,train_sequence_affect],[train_stance,train_int],shuffle=True,validation_split=0.2,epochs=20,verbose=2)

...
for train_index,test_index in kf.split(total_sequence,label_enc):
    ...
    #### shared output ##############
    shared_input=Average()([IA_stance,IA_emo])
    ...
    model=Model([input_shared,input_affect],[stance_output,emo_output])


    ##Compile
    model.compile(optimizer=Adam(0.001),loss={'task_stance':'categorical_crossentropy','task_emo':'binary_crossentropy'},
    loss_weights={'task_stance':1.0,'task_emo':0.5},metrics=['accuracy'])    
    print(model.summary())
    ...
    model.fit([train_sequence,train_sequence_affect],[train_stance,train_emo],shuffle=True,validation_split=0.2,epochs=20,verbose=2)
